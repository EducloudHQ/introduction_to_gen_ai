...
```bash
sam-gen-ai$ sam build --use-container
```
...
```bash
sam-gen-ai$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-gen-ai$ sam local start-api
sam-gen-ai$ curl http://localhost:3000/
```
...
```bash
sam-gen-ai$ sam logs -n HelloWorldFunction --stack-name sam-gen-ai --tail
```
...
```bash
sam-gen-ai$ pip install -r tests/requirements.txt --user
# unit test
sam-gen-ai$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
sam-gen-ai$ AWS_SAM_STACK_NAME="sam-gen-ai" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "sam-gen-ai"
```
